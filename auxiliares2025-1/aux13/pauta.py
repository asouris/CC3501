...
#funcion bezier
#retorna un punto en la curva definida por los puntos de control
...
if __name__ == "__main__":
    ...
    flat_pipeline = ShaderProgram(vert_program, frag_program)
    #camara
    cam = MyCam([0,6,12])

    shark = mesh_from_file(root + "/assets/shark.obj")[0]['mesh']
    ...
    def update(dt):
        ...
        nuevo_punto = bezierCurve(t, np.array(P[0]), np.array(P[1]), np.array(P[2]), np.array(P[3]))
        world["shark"]["position"] = [nuevo_punto[0], nuevo_punto[1], 0]
