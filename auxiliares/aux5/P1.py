...
void main()
{
    ...
    pipeline["view"] = pyglet.math.Mat4.look_at(
        pyglet.math.Vec3(0.0, 0.2, -0.8),  # posición inicial de la cámara            
        pyglet.math.Vec3(0, 0, 0),       # punto al que mira la cámara
        pyglet.math.Vec3(0, 1, 0))       # vector "arriba"

    #P1
    camera_positions = [
    (Vec3(0.0, 0.2, -0.8), Vec3(0, 0, 0)),        # Frontal
    (Vec3(2.0, 1.0, -1.0), Vec3(0, 0, 0)),        # Diagonal
    (Vec3(0.0, 2.0, 0.0), Vec3(0, 0, 0)),         # Desde arriba
    (Vec3(-2.0, 0.5, 2.0), Vec3(0, 0, 0)),        # Otro ángulo
    ]

    def update_camera():
        eye, target = camera_positions[window.camera_index]
    
        # Si estamos mirando desde arriba (eje Y), cambiamos el vector 'up'
        if abs(eye.y - target.y) > abs(eye.x - target.x) and abs(eye.y - target.y) > abs(eye.z - target.z):
            up = Vec3(0, 0, -1)  # El "arriba" visual será hacia -Z
        else:
            up = Vec3(0, 1, 0)
    
        pipeline["view"] = Mat4.look_at(eye, target, up)
    
        if window.is_perspective:
            pipeline["projection"] = Mat4.perspective_projection(WIDTH/HEIGHT, 0.01, 100, 90)
        else:
            scale = 2.0
            pipeline["projection"] = Mat4.orthogonal_projection(-scale, scale, -scale, scale, 0.01, 100)
    ...
    def on_key_press(symbol, modifiers):
        if symbol == key.P:
            window.is_perspective = not window.is_perspective
            update_camera()

        elif symbol == key.V:
            window.camera_index = (window.camera_index + 1) % len(camera_positions)
            update_camera()
