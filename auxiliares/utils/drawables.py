...
class Texture():
    ...
    def create_from_image(self, image):
        self.texture = texture_2D_setup(
            image, self.sWrapMode, self.tWrapMode, self.minFilterMode, self.maxFilterMode, self.flip_top_bottom)
    ...
    def init_gpu_data(self, pipeline):
        ...
        if self.index_data is not None:
            self.gpu_data = pipeline.vertex_list_indexed(
                size // count, GL_TRIANGLES, self.index_data)
        else:
