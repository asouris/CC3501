...
class MyCam(FreeCamera):
    def __init__(self, position=np.array([0, 0, 0]), camera_type="perspective"):
        ...
        self.speed = 5
        # P2: Definición de la caja contenedora AABB para el jugador
    ...
    def physics_update(self, dt):
        ...
        # P2: Sincronizar la posición del collider AABB con la cámara en cada paso físico
...
# P2: Función encargada de realizar el chequeo de colisiones contra el manager
def check_collisions(player, manager):
    collisions = manager.check_collision("player")
    if not collisions:
        return

    # De momento imprimimos las colisiones detectadas en consola
    print("Colisionando con:", *collisions)


if __name__ == "__main__":

    controller = Controller(800, 600, "Auxiliar Colisiones - P2")
    ...
    # P2: Inicialización del CollisionManager y registro del collider del jugador
    manager = colliders.CollisionManager()
    manager.add_collider(cam.collider)

    # Generación de la grilla plana
    for c in chunks:
        for z in range(MyChunk.COUNT):
            for x in range(MyChunk.COUNT):
                ...
                # P2: Si el bloque no es aire (id != 0), registramos su caja contenedora AABB
                if tipo_bloque != 0:
                    # El tamaño base del bloque es [0,0,0] a [1,1,1]
    ...
    # P2: Sincronización y actualización de las posiciones de los colisionadores en el espacio global
    for c in chunks:
        ...
        for z in range(MyChunk.COUNT):
            for x in range(MyChunk.COUNT):
                ...
                # Ajustamos la posición real del AABB sumando la transformación del chunk correspondiente
    ...
    def update(dt):
        ...
        # P2: Ejecutar el chequeo de colisiones en cada tick de la simulación
