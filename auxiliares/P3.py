...
class Controller(pyglet.window.Window):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.camera_index = 0
        self.orbit_angle = 0.0  # nuevo ángulo para cámara 360
...
void main()
{
    ...
    #Matriz perspectiva
    pipeline["projection"] = pyglet.math.Mat4.perspective_projection(WIDTH/HEIGHT, 0.01, 100, 100)
    ...
    def update(dt):
        window.time += dt
        window.orbit_angle += dt * 0.5  # velocidad angular

        radius = 0.5  # radio del círculo en que orbita la cámara
        eye_x = radius * np.sin(window.orbit_angle)
        eye_z = radius * np.cos(window.orbit_angle)
        eye = Vec3(eye_x, 0.5, eye_z)  # mantiene altura fija
        target = Vec3(0, 0.2, 0)       # punto al que siempre mira (el centro de la escena)
        up = Vec3(0, 0.5, 0)
