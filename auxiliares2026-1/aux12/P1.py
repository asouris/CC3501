#LIBRERIAS
from pyglet.graphics.shader import Shader, ShaderProgram
from pyglet.window import Window, key
from pyglet.gl import *
from pyglet.app import run
from pyglet import math
from pyglet import clock

import sys, os
import numpy as np
root = os.path.dirname(os.path.dirname((os.path.dirname(__file__))))


#MODULOS (cuidado con las rutas)
sys.path.append(os.path.dirname(os.path.dirname((os.path.dirname(__file__)))))
from utils.helpers import init_axis, mesh_from_file
from utils.camera import FreeCamera
from utils.scene_graph import SceneGraph
from utils import shapes
from utils.drawables import Texture, Model, DirectionalLight, Material, PointLight

#Controla la ventana
class Controller(Window):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.time = 0
        self.light_mode = False
        self.show_reflection = False

#CAMARA definida en una clase
class MyCam(FreeCamera):
    def __init__(self, position=np.array([0, 0, 0]), camera_type="perspective"):
        super().__init__(position, camera_type)
        self.direction = np.array([0,0,0])
        self.speed = 2

    def time_update(self, dt):
        self.update()
        dir = self.direction[0]*self.forward + self.direction[1]*self.right
        dir_norm = np.linalg.norm(dir)
        if dir_norm:
            dir /= dir_norm
        self.position += dir*self.speed*dt
        self.focus = self.position + self.forward

objects = [
    {
        "name": "cube1",
        "center": np.array([2,0,-3]),
        "half_size": np.array([0.5,0.5,0.5])
    },
    {
        "name": "cube2",
        "center": np.array([-2,0,-4]),
        "half_size": np.array([0.5,0.5,0.5])
    }
]

def ray_cube(origin, direction, center, half_size):
    box_min = center - half_size
    box_max = center + half_size

    tmin = -np.inf
    tmax = np.inf
    hit_normal = None

    for i in range(3):

        # valor muy cercano a 0 evil
        # if abs(direction[i]) < 1e-8:
        #     if origin[i] < box_min[i] or origin[i] > box_max[i]:
        #         return None

        
        t1 = (box_min[i] - origin[i]) / direction[i]
        t2 = (box_max[i] - origin[i]) / direction[i]

        if t1 < t2:
            t_near = t1
            normal = np.zeros(3)
            normal[i] = -1
            t_far = t2
        else:
            t_near = t2
            normal = np.zeros(3)
            normal[i] = 1
            t_far = t1

        if t_near > tmin:
            tmin = t_near
            hit_normal = normal

        tmax = min(tmax, t_far)

        if tmin > tmax:
            return None

    if tmax < 0:
        return None

    return tmin, hit_normal

if __name__ == "__main__":

    controller = Controller(1000, 1000,"Auxiliar 8")
    controller.set_exclusive_mouse(True)



    with open(root +  "/shaders/color_mesh_lit.vert") as f:
        color_vertex_source_code = f.read()

    with open(root +  "/shaders/color_mesh_lit.frag") as f:
        color_fragment_source_code = f.read()

    #Se define el pipeline
    vert_program = Shader(color_vertex_source_code, "vertex")
    frag_program = Shader(color_fragment_source_code, "fragment")
    pipeline = ShaderProgram(vert_program, frag_program)
    root = os.path.dirname(__file__)

    vaca = mesh_from_file(__file__ + "/../../../assets/cow.obj")[0]['mesh']
    quad = Model(shapes.Square["position"], index_data=shapes.Square["indices"], normal_data=shapes.Square["normal"])
    cube = Model(shapes.Cube["position"], index_data=shapes.Cube["indices"], normal_data=shapes.Cube["normal"])



    cam = MyCam([0,0,1])

    axis = init_axis(cam)

    graph = SceneGraph(cam)

    graph.add_node("luz",
                   pipeline=pipeline,
                   light=DirectionalLight(ambient=[0.8, 0.8, 0.8]),
                   rotation=[-np.pi/2, 0, 0])
    
    graph.add_node(
        "cube1",
        mesh=cube,
        pipeline=pipeline,
        position=[2,0,-3],
        scale=[1,1,1],
        material=Material(
            ambient=[0.3,0.3,1],
            diffuse=[0.3,0.3,1]
        )
    )

    graph.add_node(
        "cube2",
        mesh=cube,
        pipeline=pipeline,
        position=[-2,0,-4],
        scale=[1,1,1],
        material=Material(
            ambient=[1,0.7,0.2],
            diffuse=[1,0.7,0.2]
        )
    )

    graph.add_node(
        "cow",
        mesh=vaca,
        pipeline=pipeline,
        position=[0,0,-5],
        scale=[0.4]*3,
        material=Material(
            ambient=[0,0.8,0.3],
            diffuse=[0,0.8,0.3]
        ),
        
    )

    ray_vertices = np.array([
        0,0,0,
        0,0,-10
    ], dtype=np.float32)

    ray_indices = np.array([0, 1])
    ray_normals = np.array([ 0, 0, 1, 0, 0, 1], dtype=np.float32)

    ray_model = Model(ray_vertices, index_data=ray_indices, normal_data=ray_normals, mode=GL_LINES)
   
    graph.add_node(
        "ray",
        mesh=ray_model,
        pipeline=pipeline,
        material=Material(
            ambient=[1,1,0],
            diffuse=[1,1,0]
        ),
        mode=GL_LINES
    )

    ref_ray_model = Model(ray_vertices, index_data=ray_indices, normal_data=ray_normals, mode=GL_LINES)
    graph.add_node(
        "reflection",
        mesh=ref_ray_model,
        pipeline=pipeline,
        material=Material(
            ambient=[1,0,1],
            diffuse=[1,0,1]
        ),
        mode=GL_LINES,
        scale=[0, 0, 0]
    )

    @controller.event
    def on_draw():
        glClearColor(0.1, 0.1, 0.1, 1.0)
        glEnable(GL_DEPTH_TEST)
        glEnable(GL_CULL_FACE)
        glCullFace(GL_BACK)
        glFrontFace(GL_CCW)
        controller.clear()
        glClearColor(1,1,1,1)
        #axis.draw()
        graph.draw()


    #CAMARA vista en aux5
    @controller.event
    def on_key_press(symbol, modifiers):
        if symbol == key.W:
            cam.direction[0] = 1
        if symbol == key.S:
            cam.direction[0] = -1

        if symbol == key.A:
            cam.direction[1] = 1
        if symbol == key.D:
            cam.direction[1] = -1

        if symbol == key.R:
            controller.show_reflection = not controller.show_reflection

    @controller.event
    def on_key_release(symbol, modifiers):
        if symbol == key.W or symbol == key.S:
            cam.direction[0] = 0

        if symbol == key.A or symbol == key.D:
            cam.direction[1] = 0

    @controller.event
    def on_mouse_motion(x, y, dx, dy):
        cam.yaw += dx * .001
        cam.pitch += dy * .001
        cam.pitch = math.clamp(cam.pitch, -(np.pi/2 - 0.01), np.pi/2 - 0.01)

    #Informacion que se actualiza con el tiempo
    def update(dt):
        graph.update()
        axis.update()
        cam.time_update(dt)

        c_pos = cam.position.copy()
        c_pos[1] = 0

        controller.time += dt

        origin = cam.position + [0.1, -0.5, 0]
        direction = cam.forward 
        closest = 1000
        hit = None
        hit_normal = None

        for obj in objects:

            result = ray_cube(
                origin,
                direction,
                obj["center"],
                obj["half_size"]
            )

            if result is not None:
                t, normal = result

                if t < closest:
                    closest = t
                    hit = obj
                    hit_normal = normal

        end = None
        if hit is None:
            end = origin + direction * 20
        else:
            end = origin + direction*closest
            reflection = direction - 2 * np.dot(direction, hit_normal) * hit_normal
            reflection /= np.linalg.norm(reflection)

        ray_vertices = np.array([
            *origin,
            *end
        ],dtype=np.float32)

        ray_model = Model(ray_vertices, index_data=ray_indices, normal_data=ray_normals, mode=GL_LINES)
        graph["ray"]["mesh"].gpu_data.position = ray_vertices


        if hit:
            hit_point = end
            if hit["name"]=="cube1":
                graph["cube1"]["material"] = Material(
                    ambient=[1,0,0],
                    diffuse=[1,0,0]
                )
            else:
                pipeline["hit"] = 1
            
            if controller.show_reflection:
                reflection_end = hit_point + reflection * 10

                graph["reflection"]["scale"] = [1, 1, 1]
                graph["reflection"]["mesh"].gpu_data.position = np.array([
                    *hit_point,
                    *reflection_end
                ], dtype=np.float32)
            else:
                graph["reflection"]["scale"] = [0, 0, 0]
        else:
            graph["cube1"]["material"] = Material(
                ambient=[0.3,0.3,1],
                diffuse=[0.3,0.3,1]
            )
            pipeline["hit"] = 0
            graph["reflection"]["scale"] = [0, 0, 0]


    clock.schedule_interval(update,1/60)
    run()