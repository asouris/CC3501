...
class MyCam(FreeCamera):
    ...
    # P4: Método resolve_collision conservado correctamente
...
# P4: Diccionario global para la Grilla Espacial Estática
spatial_grid = {}

def check_collisions(player, manager):
    # --- 1. BROAD PHASE (Búsqueda Espacial Indexada O(1)) ---
    px, _, pz = player.position
    
    # Calculamos la celda entera del jugador en el mundo global
    center_x = int(np.floor(px))
    center_z = int(np.floor(pz))
    
    candidates = []
    # Consultamos un vecindario cerrado de 3x3 bloques en el suelo horizontal (y=0)
    ...
    player.resolve_collision(collisions)


if __name__ == "__main__":
    # Forzamos vsync=False e intervalo a 1/1000 para apreciar las métricas libres
    controller = Controller(800, 600, "Auxiliar Colisiones - P4 (Fases Broad & Narrow)", vsync=False)
    ...
    # Generación original del mundo plano extenso
    for c in chunks:
        for z in range(MyChunk.COUNT):
            for x in range(MyChunk.COUNT):
    ...
    # Sincronización y poblamiento de la grilla espacial estática
    for c in chunks:
        ...
        for z in range(MyChunk.COUNT):
            for x in range(MyChunk.COUNT):
                ...
                collider_name = f"{c.id}({x},0,{z})"
                manager.set_position(collider_name, global_pos)
                
                # Registramos las coordenadas discretas globales en la grilla rápida
