...
class SceneGraph():
    ...
    def get_transform(self, node):
        ...
        rotation_matrix = tr.rotationY(node["rotation"][1]) @ tr.rotationX(
            node["rotation"][0]) @ tr.rotationZ(node["rotation"][2])
    ...
    def draw(self):
        ...
        for src, dst in edges:
                    ...
                    if isinstance(current_node["light"], DirectionalLight):
                        if "u_dirLight.direction" in pipeline.uniforms:
                            pipeline["u_dirLight.direction"] = (
                                transformations[src] @ self.get_forward(dst))[:3]
                        ...
                        if "u_numPointLights" in pipeline.uniforms:
                            ...
                            position = (transformations[src] @ np.array([current_node["position"][0],
                                        current_node["position"][1], current_node["position"][2], 1], dtype=np.float32))[:3]
                        ...
                        if "u_numSpotLights" in pipeline.uniforms:
                            ...
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].direction"] = (
                                transformations[src] @ self.get_forward(dst))[:3]
            ...
            if current_node["mesh"] is not None:
                ...
                textured = "u_texture" in current_pipeline.uniforms and current_node[
                    "texture"] is not None
                if textured:
